    ...
    <main className="flex-1 flex flex-col bg-white">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="number" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })}
                    className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm" min="1900" max="2100" required />
                </div>
                <div>
                  ...
                  <input type="number" value={form.month} onChange={(e) => setForm({ ...form, month: e.target.value })}
                    className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm" min="1" max="12" required />
                </div>
                <div>
                  ...
                  <input type="number" value={form.day} onChange={(e) => setForm({ ...form, day: e.target.value })}
                    className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm" min="1" max="31" required />
                </div>
              </div>
              <div>
                ...
                <select value={form.shichen} onChange={(e) => setForm({ ...form, shichen: e.target.value })}
                  ...
                  {SHI_CHEN.map((s) => <option key={s.hour} value={s.hour}>{s.label}</option>)}
                </select>
              </div>
      ...
      <section className="max-w-3xl mx-auto px-4 pb-16 text-sm text-stone-600 leading-relaxed space-y-6">
        <div>
          ...
          <p>BaZi maps your birth year, month, day, and hour into four pairs of Heavenly Stems and Earthly Branches, analyzing the Five Elements at the moment of your birth to reveal your personality, career path, and life cycles.</p>
        </div>
        <div>
          <h2 className="text-lg font-semibold text-stone-800 mb-2">What is Zi Wei Dou Shu?</h2>
          <p>Zi Wei Dou Shu maps 12 life palaces — Career, Wealth, Health, Relationships — based on star positions in the Chinese celestial chart, forming the most comprehensive destiny reading system in Chinese metaphysics.</p>
        </div>
        <p className="text-xs text-stone-400 pt-4">
          <Link href="/chinese-astrology" className="hover:text-stone-600 underline">Learn more about Chinese astrology →</Link>
        </p>
      </section>
    </main>
  );
